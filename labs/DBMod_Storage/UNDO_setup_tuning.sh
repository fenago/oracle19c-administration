...
CREATE OR REPLACE EDITIONABLE TYPE "ODR"."CUST_ADDRESS_TYP"
  AS OBJECT
...
CREATE OR REPLACE EDITIONABLE TYPE "ODR"."PHONE_LIST_TYP"
  AS VARRAY(5) OF VARCHAR2(25);
/

create table odr.customers  (
 CUSTOMER_ID		NUMBER(6),
 CUST_FIRST_NAME	VARCHAR2(20),
 CUST_LAST_NAME 	VARCHAR2(20),
 CUST_ADDRESS		ODR.CUST_ADDRESS_TYP,
 PHONE_NUMBERS		ODR.PHONE_LIST_TYP,
...
create table odr.inventories (
  PRODUCT_ID     NUMBER(6),
  WAREHOUSE_ID	 NUMBER(3),
  QUANTITY_ON_HAND NUMBER(8)
 );

create table odr.product_information (
 PRODUCT_ID           NUMBER(6),
...
CREATE SEQUENCE odr.orders_seq START WITH 10 MAXVALUE 999999999999;

ALTER TABLE "ODR"."ORDERS" MODIFY ( "ORDER_TOTAL" NUMBER(12, 2) );

@/home/oracle/labs/DBMod_Storage/PERF_script_customers.sql
@/home/oracle/labs/DBMod_Storage/PERF_script_product_information.sql
drop table odr.lineorder;
create table odr.lineorder
(LO_ORDERKEY number,
...
drop table odr.supplier;
create table odr.supplier (
S_SUPPKEY number,
S_NAME char(25),
...
drop table odr.customer;
create table odr.customer (
C_CUSTKEY number,
C_NAME varchar(25),
...
drop table odr.date_dim;
create table odr.date_dim (
D_DATEKEY number,
D_DATE char(18),
D_DAYOFWEEK char(10),
D_MONTH char(9),
...
insert /*+ append */ into odr.part  select p_partkey + 200001, P_NAME,P_MFGR,
				  ...
				  FROM odr.part;
commit;
insert /*+ append */ into odr.part  select p_partkey + 400002, P_NAME,P_MFGR,
                  P_CATEGORY,P_BRAND1,P_COLOR,P_TYPE,P_SIZE,P_CONTAINER
				  FROM odr.part;
commit;
insert /*+ append */ into odr.part  select p_partkey + 800004, P_NAME,P_MFGR,
                  P_CATEGORY,P_BRAND1,P_COLOR,P_TYPE,P_SIZE,P_CONTAINER
				  FROM odr.part;
commit;
...
commit;	 
insert  /*+ append */ into odr.supplier select S_SUPPKEY + 4002, S_NAME, S_ADDRESS, S_CITY , S_NATION, S_REGION	, S_PHONE from odr.supplier;
commit;	 
insert  /*+ append */ into odr.supplier select S_SUPPKEY + 8004, S_NAME, S_ADDRESS, S_CITY , S_NATION, S_REGION	, S_PHONE from odr.supplier;
commit;	 

insert  /*+ append */ into odr.customer select C_CUSTKEY+30001, C_NAME ,C_ADDRESS, C_CITY , C_NATION, C_REGION,C_PHONE,C_MKTSEGMENT	from odr.customer;
commit;
insert  /*+ append */ into odr.customer select C_CUSTKEY+60002, C_NAME ,C_ADDRESS, C_CITY , C_NATION, C_REGION,C_PHONE,C_MKTSEGMENT	from odr.customer;
commit;
insert  /*+ append */ into odr.customer select C_CUSTKEY+120004, C_NAME ,C_ADDRESS, C_CITY , C_NATION, C_REGION,C_PHONE,C_MKTSEGMENT from odr.customer;
commit;

insert /*+ append */ into odr.lineorder select 
